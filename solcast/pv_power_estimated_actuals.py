...
class PvPowerEstimatedActuals(Base):
    ...
    def _generate_est_acts_dict(self):
        ...
        for est_act in self.content.get('estimated_actuals'):
            parsed_est_act = {'period_end' : parse_datetime(est_act['period_end']),
                               'period' : parse_duration(est_act['period']),
                               'pv_estimate' : est_act['pv_estimate']
                              }

            self.estimated_actuals.append(parsed_est_act)
